...
import {
  FileCode, Save, Plus, Trash2, Edit3, Check, X, Star, Loader2,
} from "lucide-react";

export function AdvancedModeEditor() {
  const { user } = useAuth();
  ...
  const handleSave = async () => {
    if (!latexCode.trim()) {
      toast({ title: "Template required", description: "Please enter your LaTeX template code.", variant: "destructive" });
      return;
    }
    setIsSaving(true);
    try {
      if (isCreating) {
        ...
        const success = await updateTemplate(editingTemplate.id, { name: name || "My Template", latex_code: latexCode, is_default: isDefault });
        if (success) {
  ...
  const handleDelete = async (template: Template) => {
    ...
    if (success) {
      ...
      if (editingTemplate?.id === template.id) handleCancel();
    } else {
        ...
        {loading ? (
          ...
          <AnimatePresence>
            {templates.map((template, index) => (
              ...
              >
                <Card
                  className={`cursor-pointer transition-all hover:border-primary/50 ${editingTemplate?.id === template.id ? "border-primary ring-1 ring-primary" : ""}`}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {template.is_default && <Star className="h-3.5 w-3.5 text-primary fill-primary flex-shrink-0" />}
                        </div>
          ...
          {isEditing ? (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="My Resume Template" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="latex">LaTeX Code</Label>
                    <Textarea id="latex" value={latexCode} onChange={(e) => setLatexCode(e.target.value)} placeholder="Paste your LaTeX template code here..." className="min-h-[300px] font-mono text-xs" />
                  </div>
                  <div className="flex items-center gap-3">
                    <Switch id="default" checked={isDefault} onCheckedChange={setIsDefault} />
                    <Label htmlFor="default" className="text-sm">Set as default template for AI generation</Label>
                  </div>
            ...
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <Card className="p-12 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-4">Select a template to edit or create a new one</p>
